...
import com.austgl.syllabus.adapter.UpLoadCourseAdapter;
...
public class UpLoadedActivity extends Activity implements OnClickListener {
	private static final String LOGTAG = LogUtil
			.makeLogTag(UpLoadedActivity.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		executorService = Executors.newSingleThreadExecutor(); // single thread
																// exector
		// setExecutorService(Executors.newSingleThreadExecutor()); // single
		// thread exector
	...
	private void initData() {
		...
		adapterForAdd = new UpLoadCourseAdapter(courses_Add,
				course_add_selected, this);
		adapterForUpdate = new UpLoadCourseAdapter(courses_Update,
				course_update_selected, this);
	...
	public void onClick(View arg0) {
		...
		case R.id.tvRightT:
			for (int i = 0; i < course_add_selected.size(); i++) {
				System.out.println("UpLoadedActivity---course_add_selected["
						+ i + "]:" + course_add_selected.get(i));
			}
			...
			handler.post(new Runnable() {
				...
				public void run() {
					if (executorService.isTerminated()) {
						pd.dismiss();
						initData();
						Toast.makeText(UpLoadedActivity.this, "同步完成！您的课表已是最新",
	...
	private List<NameValuePair> configureValues(Course course) {
		...
		for (int j = 0; j < courseValues.length; j++) {
			NameValuePair value = new BasicNameValuePair(FIELDS[j],
					URLEncoder.encode(courseValues[j]));
			values.add(value);
		}
		return values;
	}
			...
			try {
				...
				if (null != s && !"".equals(s)) {
					...
					if (UPDATE_COURSE == resultCode) {
						...
						UnUploadedCourseDaoImpl unDao = new UnUploadedCourseDaoImpl(
								UpLoadedActivity.this);
						unDao.deleteCourse(course.getId(),
								CommonConstants.UNDO_ACTION_UPDATE);
					} else {
	...
	public class TaskSubmitter {
		...
		public Future submit(Runnable task) {
			Future result = null;
			if (!activity.getExecutorService().isTerminated()
					&& !activity.getExecutorService().isShutdown()
					&& task != null) {
				result = activity.getExecutorService().submit(task);
			}
			return result; // if result.get() returns null, it means that the
	...
	private void addTask(Runnable runnable) {
		...
		synchronized (taskList) {
			...
			if (taskList.isEmpty() && !running) // running is false when first
												// register
			{
				running = true;
				futureTask = taskSubmitter.submit(runnable); // submit the
																// runnable
																// object to the
																// ExecutorService.
				if (futureTask == null) // if futureTask is null, it means that
				...
				Log.v(LOGTAG,
						"task is not empty or is running" + runnable.toString());

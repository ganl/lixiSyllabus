...
public class PhoneStateService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (isShieldOpen) {
			if (callEnded) {
				...
				boolean isInClass = CommonConstants.isDuringCourseTime(
			...
			{// 如果应用启用，执行监听电话状态改变操作
				...
				{// 来电
					Log.i("PhoneStateService",
							"------in --- CALL_STATE_RINGING isCallOut "
				...
				{// 根据newOutGoing判断是否是拨出电话
					Log.i("PhoneStateService",
							"------in --- CALL_STATE_OFFHOOK isCallOut "
									+ " incomingNumber " + incomingNumber
									+ ",callOutPhoneNum" + callOutPhoneNum);
					lastState = TelephonyManager.CALL_STATE_OFFHOOK;
				} else if (state == TelephonyManager.CALL_STATE_IDLE) // == 0
				{// 挂断电话
					Log.i("PhoneStateService",
							"------in --- CALL_STATE_IDLE isCallOut "
									+ " incomingNumber " + incomingNumber);
	...
	// 2.3来电接听方法
	public synchronized void answerRingingCall(Context context) {// 据说该方法只能用于Android2.3及2.3以上的版本上
		try {
	...
	public void notifyIncomingCallAndCallBack(Context context) {
		...
		// context.getSharedPreferences(CommonConstants.SHARED_NAME,
		...
		notification.setLatestEventInfo(context, contentTitle, contentText,
				contentIntent);
		nm.notify(((SyllabusApplication) getApplication()).notificationID,
				notification);
	...
	class MyHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				// PendingIntent
				// intent1=PendingIntent.getBroadcast(MyShowActivity.this, 0,
				// new Intent(), 0);
				manager.sendTextMessage(incomingNumber, null, share.getString(
						CommonConstants.SMS_TEXT,
						CommonConstants.DEFAULT_SMSTEXT), null, null);
				Log.i("PhoneStateService", "message has been sent to "
						+ incomingNumber);
			...
			case 2:
				...
				if (mAudioManager.getRingerMode() == AudioManager.RINGER_MODE_SILENT) {
					mAudioManager
							.setRingerMode(CommonConstants.defaultRingerMode);
